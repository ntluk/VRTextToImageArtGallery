...
namespace Tobii.XR
{
    ...
    public class UIGazeButtonGraphics : MonoBehaviour
    {
        ...
        private IEnumerator AnimateButton(float duration, AnimationCurve animationCurve, ButtonState currentButtonState)
        {
            ...
            switch (currentButtonState)
            {
                case ButtonState.Focused:
                    ...
                    //Deactive menuStatus and gaze line when an icon is focused
                    if (menuStatus != null)
                    {
                ...
                case ButtonState.Idle:
                    if (menuStatus != null)
                    {
                        if (bookIcon != null && settingIcon != null && hologramIcon != null)
                        {
                            bookIcon.GetComponent<BoxCollider>().enabled = false;
                            settingIcon.GetComponent<BoxCollider>().enabled = false;
                            hologramIcon.GetComponent<BoxCollider>().enabled = false;
                            Invoke(nameof(ReactivateIcons), 0.1f);
                        }
        ...
        {
            bookIcon.GetComponent<BoxCollider>().enabled = true;
            settingIcon.GetComponent<BoxCollider>().enabled = true;
            hologramIcon.GetComponent<BoxCollider>().enabled = true;
        }
    }    
}